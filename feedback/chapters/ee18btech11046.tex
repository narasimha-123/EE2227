...
The following netlist simulates dc analysis of circuit which gives $I_{c}$ vs $V_{be}$ characteristics of bjt.
\begin{lstlisting}
codes/ee18btech11046/spice/ee18btech11046_spice2.net
\end{lstlisting}

From $I_{c}$ vs $V_{be}$ slope at Q-point(middle point of linear region) gives $g_{m}$
\begin{align}
g_{m} = \frac{\partial I_{c}}{\partial V_{be}} = 0.0104943S
\end{align}
...
The following netlist simulates dc analysis of circuit which gives $V_{be}$ vs $I_{b}$ characteristics of bjt.
\begin{lstlisting}
codes/ee18btech11046/spice/ee18btech11046_spice3.net
\end{lstlisting}

From $V_{be}$ vs $I_{b}$ slope at Q-point(middle point of linear region) gives $r_{\pi}$
\begin{align}
r_{\pi} = \frac{\partial V_{be}}{\partial I_{b}} = 9528.921\ohm
\end{align}

The $I_{C}$ vs $V_{be}$ characteristics obtaines from spice are plotted in  fig.\ref{fig:ee18btech11046_spice3}
%
\begin{figure}[!ht]
\centering
...
\end{lstlisting}
We can observe that $V_{o}$ is sum of input sine wave amplified by a factor of 89500 for small signal input and large signal output $V_{C}$ which is close to the calculated values.
